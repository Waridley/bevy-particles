...
impl Linear {
	pub fn tick(mut q: Query<(&Self, &mut Transform, &InitialTransform, &TimeCreated)>, t: Res<Time>) {
		for (item, mut xform, init_xform, t_created) in &mut q {
			xform.translation = init_xform.translation +
				(item.velocity * t.last_update().unwrap().duration_since(t_created.0).as_secs_f32());
		}
	}
}
	...
	pub fn tick(mut q: Query<(&Self, &mut Transform, &InitialTransform, &TimeCreated, &Lifetime)>, t: Res<Time>) {
		for (target, mut xform, init_xform, t_created, lifetime) in &mut q {
			xform.scale = init_xform.scale.lerp(
				target.scale,
				t.last_update().unwrap().duration_since(t_created.0).as_secs_f32() / lifetime.0.as_secs_f32()
			)
		}
	}
}
